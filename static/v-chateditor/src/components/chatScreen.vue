  ...
  <div class="chatEditor_" id="chatEditor_" :style="chatEditorStyle||''">
    <div style="width:100%;height:100%;min-width:300px;position: relative;text-align: left;">
      ...
      <div id="expression_" class="expression_">
        <!-- 切换 -->
        <div class="expressionSwitch">
          <a class="qq_bq bqqh">表情</a>
        </div>
        <!-- 表情展示 -->
        <div class="bqShow">
          <div id="bqShow_k" class="bqShow_k"></div>
        </div>
      </div>
      <!--功能按钮区-->
      <div
        :style="'width:100%;height:36px;box-sizing: border-box;padding: 3px;'+(btnAreaStyle||'')"
      >
      ...
      <div style="width:100%;height:30px;box-sizing: border-box;text-align: right;">
        <button
          ...
          :style="sendBtnStyle||''"
        >长按录音</button>
        <button id="sendMsg" class="sendMsg" @click="sendMsgClick" :style="sendBtnStyle||''">发送</button>
        <button id="sendVideo" class="sendVideo" style="display:none;" @click="sendVideo">视频发送</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      for (var i = 0, len = e.clipboardData.items.length; i < len; i++) {
        ...
        if (item.kind === "string") {
          item.getAsString(function (str) {
            // str 是获取到的字符串
            var preElOth = document.getElementById("preId");
            var str = preElOth.innerText;
            preElOth.innerText = str;
          });
        } else if (item.kind === "file") {
          var pasteFile = item.getAsFile();
          // pasteFile就是获取到的文件
          var files = { 0: pasteFile }
    ...
    uploadFile() {
      var files = chatScreen.uploadFile();
      this.$emit("sendFiles", files);
    },
    startRecording() {
    ...
    preIdKeydown: chatScreen.preIdKeydown((msgArr, fileArr) => {
      ...
      if (atView && atView.style.display == "block") {
        return false;
      }
